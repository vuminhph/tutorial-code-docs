...
2.  **Node:** A single step or "station" in the workflow. Each node has a specific type and function:
    ...
    - _(And others like `Report`, `QA Retriever`)_
  ...
  "nodes": [
    ...
    },
    {
      ...
      "data": {
        "type": "LLM",
        "name": "Brainstorm",
        "config": { "prompt": "Points for {input_topic.output}" }
      }
    },
    // ... other nodes ...
    {
      ...
      "data": {
        "type": "Output",
        "name": "Show Summary",
        "config": { "value": "{llm_summarize.output}" }
      }
    },
...
- `_run`: This is the _abstract_ method where the unique logic for each node type (LLM, Tool, Code) lives. Subclasses _must_ implement this.
...
This engine provides the structure for defining _what_ the steps are and _how_ they connect. But how does the underlying execution engine actually manage the state transitions and potentially complex cycles or conditional paths within the graph?
